...
describe("runCheckDrift", () => {
  ...
  it("should return exitCode and reportPath from result", async () => {
    checkForDrift.mockResolvedValue({
      exitCode: 0,
      result: { driftDetected: false, comparisonSupported: true, reportPath: "custom-report.html" },
    });
  ...
  it("should set GitHub outputs when drift detected", async () => {
    checkForDrift.mockResolvedValue({
      exitCode: 1,
      result: { driftDetected: true, comparisonSupported: true, driftResolutionFolder: "/resolution" },
    });
  ...
  it("should not set outputs when comparison not supported", async () => {
    checkForDrift.mockResolvedValue({ exitCode: 0, result: { driftDetected: undefined, comparisonSupported: false } });
  ...
  it("should not set outputs when non-drift error occurs", async () => {
    checkForDrift.mockResolvedValue({ exitCode: 1, result: { driftDetected: false, comparisonSupported: true } });
