...
| Input                              | Description                                                 | Required | Default              |
|------------------------------------|-------------------------------------------------------------|----------|----------------------|
| `deployment-report-name`           | Name for the deployment report artifact                     | No       | `flyway-undo-report` |
...
If the 'undo' action runs more than once in the same workflow (e.g. against multiple target databases), use a unique `deployment-report-name` for each run to avoid artifact name conflicts:
...
steps:
  - uses: red-gate/flyway-actions/migrations/undo@v1
    with:
      ...
      deployment-report-name: flyway-undo-report-${{ matrix.target }}
```
