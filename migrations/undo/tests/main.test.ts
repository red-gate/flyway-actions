...
type SetupFlywayMockOptions = {
  ...
  undoExitCode: number;
  undoOutput?: Record<string, unknown>;
};
...
describe("run", () => {
  ...
  const setupFlywayMock = ({
    ...
    calls.push({ stdout: undoOutput, exitCode: undoExitCode });
  ...
  it("should fail when neither url nor environment is provided", async () => {
    setupFlywayMock({ edition: "Community", undoExitCode: 0 });
  ...
  it("should fail when flyway returns non-zero exit code", async () => {
    ...
    expect(setFailed).toHaveBeenCalledWith(expect.stringContaining("Flyway undo failed with exit code 1"));
  });
  ...
  it("should set outputs on successful execution", async () => {
    setupFlywayMock({
      edition: "Community",
      undoExitCode: 0,
      undoOutput: { migrationsUndone: 3, targetSchemaVersion: "3" },
    });
    getInput.mockImplementation((name: string) => {
      if (name === "target-url") {
        return "jdbc:sqlite:test.db";
  ...
  it("should fail and not undo when drift is detected for enterprise edition", async () => {
    setupFlywayMock({ edition: "Enterprise", driftExitCode: 1, undoExitCode: 0 });
    getInput.mockImplementation((name: string) => {
      if (name === "target-url") {
        return "jdbc:sqlite:test.db";
      }
  ...
  it("should proceed with undo when no drift detected for enterprise edition", async () => {
    setupFlywayMock({
      edition: "Enterprise",
      driftExitCode: 0,
      undoExitCode: 0,
