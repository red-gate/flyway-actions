...
const run = async (): Promise<void> => {
  try {
    ...
    if (flywayDetails.edition === "enterprise") {
      if (inputs.skipDriftCheck) {
        ...
        if (driftResult.driftDetected) {
          driftDetected = true;
          await writeSummary({ driftChecked, driftDetected, migrationsApplied: 0, schemaVersion: "unknown" });
    ...
    await writeSummary({ driftChecked, driftDetected, migrationsApplied, schemaVersion });
  } catch (error) {
