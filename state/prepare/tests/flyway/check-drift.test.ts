...
describe("runCheckDrift", () => {
  beforeEach(() => {
    checkForDrift.mockResolvedValue({ exitCode: 0, result: { driftDetected: false, comparisonSupported: true } });
  });
  ...
  it("should include report filename when preDeploymentReportName is set", async () => {
    await runCheckDrift({ preDeploymentReportName: "my-report" });
  ...
  it("should not include report filename when preDeploymentReportName is not set", async () => {
    await runCheckDrift({});
  ...
  it("should return result from checkForDrift", async () => {
    checkForDrift.mockResolvedValue({ exitCode: 1, result: { driftDetected: true, comparisonSupported: true } });

    const result = await runCheckDrift({});

    expect(result).toEqual({ exitCode: 1, result: { driftDetected: true, comparisonSupported: true } });
  });
